...
const setActive = (index) => {
  ...
  if (index === activeSection.value.length) active = 0;
  else if (index === -1) active = activeSection.value.length - 1;
...
<div class="pattern bg-amber-50">
    <div class="container flex flex-col px-6 py-4 mx-auto space-y-6 lg:h-[32rem] lg:py-16 lg:flex-row lg:items-center">
        <div :class="{'lg:w-1/2': activeSection}" class="flex flex-col items-center w-full lg:flex-row ">
            <div class="flex justify-center order-2 mt-6 lg:mt-0 lg:space-y-3 lg:flex-col">
                <button 
                    ...
                    @click="setActive(idx)"
                    class="w-3 h-3 mx-2  rounded-full lg:mx-0 focus:outline-none" ></button>
            </div>

            <div v-if="activeSection" class="max-w-lg lg:mx-12 lg:order-2">
                ...
                <div class="mt-6">
                    <a href="#" class="px-6 py-2.5 mt-6 text-sm font-medium leading-5 text-center text-white capitalize bg-amber-400 rounded-full hover:bg-amber-300 lg:mx-0 lg:w-auto focus:outline-none">Download from App Store</a>
                </div>
            </div>

            <div v-else class=" text-center lg:mx-12 lg:order-2">
                <Image :src="appUseUtils.getImgUrl(sections[activeSection].itemImageSrc)" alt="Image" width="150" />
                <h1 class="text-3xl font-semibold tracking-wide text-gray-900 dark:text-white lg:text-4xl">{{sections[activeSection].title}}</h1>
                <p class="mt-4 text-gray-600 dark:text-gray-300">{{ sections[activeSection].subtitle }}</p>
                <div class="mt-6">
                    <a href="#" class="px-6 py-2.5 mt-6 text-sm font-medium leading-5 text-center text-white capitalize bg-amber-400 rounded-full hover:bg-amber-300 lg:mx-0 lg:w-auto focus:outline-none">Download from App Store</a>
                </div>
